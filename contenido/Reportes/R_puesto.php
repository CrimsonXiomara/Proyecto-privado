<?php
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'PUESTO ELECTORAL');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('D1', 'PARTIDO');
	...
	foreach($result as $row){
			...
			$id_puesto = $row['id_puesto_electoral'];
			$desc_puesto= $row['nom_puesto'];
			$candidato_nom = " ";

			$sql1 = "select C.p_nombre, C.s_nombre, C.p_ape, C.s_ape, C.numero_de_identificacion, F.fecha, PA.desc_partido ";
			...
			$sql1.= "and C.id_puesto = $id_puesto;";
			...
			foreach ($res as $mostrar) {
				...
				$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $desc_puesto);
				...
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, $mostrar['desc_partido']);
